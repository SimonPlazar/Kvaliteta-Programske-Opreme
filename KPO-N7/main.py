...
LOG_DIR = DataLogger.DEFAULT_LOG_DIR
LOG_INTERVAL_TICKS = DataLogger.DEFAULT_LOG_INTERVAL_TICKS
...
class Terrain:
    ...
    def _generate_random_grid(self):
        ...
        raw_grid = generate_terrain_grid(
            ...
            sand_threshold=0.425,
            grass_threshold=0.775,
            forest_threshold=0.925,
            mountain_threshold=0.975,
        )
...
class SimThread(threading.Thread):
    def __init__(self, data_queue: queue.Queue, n_prey: int, n_pred: int, n_food: int, init_params: dict,
                 simulation_delay: float, terrain_file: str = None, log_enabled: bool = True,
                 log_dir: str = None, log_interval_ticks: int = None):
        ...
        self.log_interval_ticks = DataLogger.DEFAULT_LOG_INTERVAL_TICKS
        if log_enabled:
            self.log_interval_ticks = int(log_interval_ticks or DataLogger.DEFAULT_LOG_INTERVAL_TICKS)
            ...
            Agent.death_hook = self._on_agent_death

        self._last_log_tick = 0
        self._last_dropped_frames = 0
        self._interval_births = {"prey": 0, "pred": 0, "clover": 0}
        self._interval_deaths = {
            "total": 0,
            "thirst": 0,
            "age": 0,
            "predation": 0,
    ...
    def _step(self):
        ...
        self._register_births(new_agents)
        self.agents.extend(new_agents)
        self.agents = [a for a in self.agents if a.alive]

        if random.random() < 0.005:
            clover = self._spawn_clover()
            self._register_births([clover])
            self.agents.append(clover)

        self._log_interval_if_needed()
    ...
    def _on_agent_death(self, agent, reason):
        self._interval_deaths["total"] += 1

        if reason in self._interval_deaths:
            self._interval_deaths[reason] += 1
        else:
            self._interval_deaths["other"] += 1

    def _register_births(self, new_agents):
        for ag in new_agents:
            if ag.entity_type is EntityType.PREY:
                self._interval_births["prey"] += 1
            elif ag.entity_type is EntityType.PREDATOR:
                self._interval_births["pred"] += 1
            elif ag.entity_type is EntityType.CLOVER:
                self._interval_births["clover"] += 1

    def _log_interval_if_needed(self):
        if not self.data_logger:
            return

        interval_ticks = self.ticks - self._last_log_tick
        if interval_ticks < self.log_interval_ticks:
            return

        stats = self._compute_population_stats()
        dropped_delta = self.dropped_frames - self._last_dropped_frames

        row = {
            "tick": self.ticks,
            "interval_ticks": interval_ticks,
            "total_agents": stats["total_agents"],
            "prey_cnt": stats["prey_cnt"],
            "pred_cnt": stats["pred_cnt"],
            "clover_cnt": stats["clover_cnt"],
            "births_total": sum(self._interval_births.values()),
            "births_prey": self._interval_births["prey"],
            "births_pred": self._interval_births["pred"],
            "births_clover": self._interval_births["clover"],
            "deaths_total": self._interval_deaths["total"],
            "deaths_thirst": self._interval_deaths["thirst"],
            "deaths_age": self._interval_deaths["age"],
            "deaths_predation": self._interval_deaths["predation"],
            "deaths_consumed": self._interval_deaths["consumed"],
            "deaths_other": self._interval_deaths["other"],
            "dropped_frames": dropped_delta,
            "avg_prey_hunger": self._safe_avg(stats["prey_hunger_sum"], stats["prey_cnt"]),
            "avg_prey_thirst": self._safe_avg(stats["prey_thirst_sum"], stats["prey_cnt"]),
            "avg_prey_speed": self._safe_avg(stats["prey_speed_sum"], stats["prey_cnt"]),
            "avg_prey_size": self._safe_avg(stats["prey_size_sum"], stats["prey_cnt"]),
            "avg_prey_sense": self._safe_avg(stats["prey_sense_sum"], stats["prey_cnt"]),
            "avg_prey_age": self._safe_avg(stats["prey_age_sum"], stats["prey_cnt"]),
            "avg_pred_hunger": self._safe_avg(stats["pred_hunger_sum"], stats["pred_cnt"]),
            "avg_pred_thirst": self._safe_avg(stats["pred_thirst_sum"], stats["pred_cnt"]),
            "avg_pred_speed": self._safe_avg(stats["pred_speed_sum"], stats["pred_cnt"]),
            "avg_pred_size": self._safe_avg(stats["pred_size_sum"], stats["pred_cnt"]),
            "avg_pred_sense": self._safe_avg(stats["pred_sense_sum"], stats["pred_cnt"]),
            "avg_pred_age": self._safe_avg(stats["pred_age_sum"], stats["pred_cnt"]),
            "avg_clover_thirst": self._safe_avg(stats["clover_thirst_sum"], stats["clover_cnt"]),
            "avg_clover_age": self._safe_avg(stats["clover_age_sum"], stats["clover_cnt"]),
        }

        self.data_logger.enqueue_row(row)
        self._last_log_tick = self.ticks
        self._last_dropped_frames = self.dropped_frames
    ...
    def _compute_population_stats(self):
        ...
        for ag in self.agents:
            if ag.entity_type is EntityType.PREY:
                stats["prey_cnt"] += 1
                stats["prey_hunger_sum"] += ag.hunger
                stats["prey_thirst_sum"] += ag.thirst
                stats["prey_speed_sum"] += ag.speed
                stats["prey_size_sum"] += ag.size
                stats["prey_sense_sum"] += ag.sense
                stats["prey_age_sum"] += ag.age
            elif ag.entity_type is EntityType.PREDATOR:
                stats["pred_cnt"] += 1
                stats["pred_hunger_sum"] += ag.hunger
                stats["pred_thirst_sum"] += ag.thirst
                stats["pred_speed_sum"] += ag.speed
                stats["pred_size_sum"] += ag.size
                stats["pred_sense_sum"] += ag.sense
                stats["pred_age_sum"] += ag.age
            elif ag.entity_type is EntityType.CLOVER:
                stats["clover_cnt"] += 1
                stats["clover_thirst_sum"] += ag.thirst
                stats["clover_age_sum"] += ag.age

        return stats

    def _safe_avg(self, total, count):
    ...
    def _close_logger(self):
        ...
        self.data_logger.close({
            "ticks": self.ticks,
            "last_dropped_frames": self.dropped_frames,
        })
        self.data_logger = None
        Agent.death_hook = None
...
class App(tk.Tk):
    ...
    def _build_settings(self):
        ...
        self._add_logging_settings_row(outer)
    ...
    def _add_logging_settings_row(self, parent):
        row = tk.Frame(parent)
        row.pack(fill=tk.X, pady=5)

        self.chk_enable_logging = tk.Checkbutton(
            row,
            text="Enable Logging",
            variable=self.enable_logging,
            command=self._toggle_logging_options,
        )
        self.chk_enable_logging.pack(side=tk.LEFT, padx=(5, 10))

        self.logging_options_frame = tk.Frame(row)
        self.logging_options_frame.pack(side=tk.LEFT)

        tk.Label(self.logging_options_frame, text="Log Dir:").pack(side=tk.LEFT)
        self.entry_log_dir = tk.Entry(self.logging_options_frame, width=22)
        self.entry_log_dir.insert(0, DataLogger.DEFAULT_LOG_DIR)
        self.entry_log_dir.pack(side=tk.LEFT, padx=(4, 6))

        self.btn_browse_log_dir = tk.Button(
            self.logging_options_frame,
            text="Browse...",
            width=8,
            command=self._browse_log_dir,
        )
        self.btn_browse_log_dir.pack(side=tk.LEFT, padx=(0, 10))

        tk.Label(self.logging_options_frame, text="Interval (ticks):").pack(side=tk.LEFT)
        self.entry_log_interval = tk.Entry(self.logging_options_frame, width=6)
        self.entry_log_interval.insert(0, str(DataLogger.DEFAULT_LOG_INTERVAL_TICKS))
        self.entry_log_interval.pack(side=tk.LEFT, padx=(4, 0))

        self._toggle_logging_options()

    def _browse_log_dir(self):
        current_dir = self.entry_log_dir.get().strip() or DataLogger.DEFAULT_LOG_DIR
        selected = filedialog.askdirectory(initialdir=current_dir, title="Select log folder")
        if not selected:
            return
        self.entry_log_dir.delete(0, tk.END)
        self.entry_log_dir.insert(0, selected)

    def _toggle_logging_options(self):
        if self.enable_logging.get():
            if not self.logging_options_frame.winfo_ismapped():
                self.logging_options_frame.pack(side=tk.LEFT)
            return

        if self.logging_options_frame.winfo_ismapped():
    ...
    def _update_parameter_entry_states(self, is_running):
        ...
        if hasattr(self, "chk_enable_logging"):
            self.chk_enable_logging.config(state=state)

        if hasattr(self, "entry_log_dir"):
            self.entry_log_dir.config(state=state)

        if hasattr(self, "btn_browse_log_dir"):
            self.btn_browse_log_dir.config(state=state)

        if hasattr(self, "entry_log_interval"):
            self.entry_log_interval.config(state=state)
    ...
    def start(self):
        ...
        log_enabled = self.enable_logging.get()
        log_dir = None
        log_interval_ticks = None

        if log_enabled:
            log_dir = self._read_log_dir()
            log_interval_ticks = self._read_log_interval_ticks()
        ...
        self.sim = SimThread(
            ...
            log_dir=log_dir,
            log_interval_ticks=log_interval_ticks,
        )
        self.sim.start()
    ...
    def _read_log_dir(self):
        if not hasattr(self, "entry_log_dir"):
            return None
        value = self.entry_log_dir.get().strip()
        return value or None

    def _read_log_interval_ticks(self):
        if not hasattr(self, "entry_log_interval"):
            return None
        try:
            return int(self.entry_log_interval.get())
        except ValueError:
            return DataLogger.DEFAULT_LOG_INTERVAL_TICKS
